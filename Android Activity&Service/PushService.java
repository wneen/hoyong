...
public class PushService extends Service
{
	...
	private static final String		MQTT_HOST = "192.168.219.141";
	...
	private SharedPreferences 		mPretopic;
	...
	public static final String       PREF_TOPIC= "preftopic";
	public static final String       PREF_HOST= "192.168.219.141";
	public static final String       PREF_PORT= "1883";
	// Notification title
	public static String			NOTIF_TITLE = "MqttTest"; 
	// Notification id
	...
	private synchronized void connect() {		
		...
		if (deviceID == null) {
			...
			try {
				mPretopic = getSharedPreferences(PREF_TOPIC, MODE_PRIVATE);
				String host = mPretopic.getString(PushService.PREF_HOST, "192.168.0.0");
	...
	private void showNotification(String text) {
		...
		n.flags |= Notification.FLAG_SHOW_LIGHTS; // LED불빛을 출력
      	n.flags |= Notification.FLAG_AUTO_CANCEL; //	아이콘탭하면 자동으로 통지 취소

        n.defaults = Notification.DEFAULT_ALL;//DEFAULT_SOUND  	소리를 발생   
        											//DEFAULT_VIBRATE  	진동을 발생   
        											//DEFAULT_LIGHTS  	불빛을 깜박거림을 다 사용
      	
		n.icon = com.kasund.mqtt.demo.R.drawable.icon; //아이콘 모양
		n.when = System.currentTimeMillis(); //표시할 시간

		// Simply open the parent activity
		PendingIntent pi = PendingIntent.getActivity(this, 0,  // 인텐트를 래핑하며 다른 응용 프로그램으로 전달하여 실행 권한을 준다. 이는 시스템이 관리하며 인텐트를 만든 응용 프로그램이 종료되어도 유효
		  new Intent(this, PushActivity.class), 0);

		// Change the name of the notification here
		n.setLatestEventInfo(this, NOTIF_TITLE, text, pi);//Notification List 표시등록
	...
	private class MQTTConnection implements MqttSimpleCallback {
		...
		public MQTTConnection(String brokerHostName, String initTopic) throws MqttException {
			...
			mPretopic = getSharedPreferences(PREF_TOPIC, MODE_PRIVATE);
			String PORT = mPretopic.getString(PushService.PREF_PORT, "1883");
			...
			String mqttConnSpec = "tcp://" + brokerHostName + "@" + PORT;
				...
				String Topic ="good";
				mPretopic = getSharedPreferences(PREF_TOPIC, MODE_PRIVATE);
				String topic = mPretopic.getString(PushService.PREF_TOPIC, "good");
				subscribeToTopic(topic);
		
				log("Connection established to " + brokerHostName + " on topic " + topic);
		...
		/*
		 * Called if the application loses it's connection to the message broker.MQTT_BROKER_PORT_NUM
		 */
		...
		public void publishArrived(String topicName,byte[] payload, int qos, boolean retained) {
			...
			message=new String(payload,0,payload.length);
			handler.post(new Runnable() {
			    public void run() {
			        Toast toast = Toast.makeText(PushService.this, message, Toast.LENGTH_LONG);
			        toast.show();
			    }
			 });
		}   
